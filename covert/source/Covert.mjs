...
const Covert = function (settings) {
  ...
  this.on("connect", (reviews) => {
    if (this.__state.connected === false) {
      let need_network =
        this.reviews.filter((review) => review.flags.network === true).length >
        0;
      let need_internet =
        this.reviews.filter((review) => review.flags.internet === true).length >
        0;
...
Covert.prototype = Object.assign({}, Emitter.prototype, {
  ...
  connect: function () {
    ...
    if (review !== null && test !== null) {
      ...
      this.network.check(process.env.PORT, (result) => {
        if (result === false) {
          required_ports.push(process.env.PORT);
        }
      });
      ...
      if (this._settings.inspect !== null) {
        console.error(
          'Covert: No Review(s) match the patterns "' +
            this._settings.patterns.join('" or "') +
            '" that contain a Test "' +
            this._settings.inspect +
            '".'
        );
      } else {
        console.error(
