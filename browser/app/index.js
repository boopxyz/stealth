...
const FLAGS = (() => {
  let flags = {
    ...
    host: null,
    profile: null,
    app: "http://localhost:process.env.PORT/browser/index.html",
    "user-data-dir": "/tmp/tholian-browser",
  };

  Array.from(process.argv)
    .filter((v) => v.startsWith("--") === true)
    .forEach((flag) => {
  ...
  Array.from(process.argv)
    .filter((v) => v.startsWith("-") === true && v.startsWith("--") === false)
    .forEach((flag) => {
      if (flag === "-h" || flag === "-help") {
        flags.help = true;
      }
...
if (FLAGS.help === true) {
  ...
  app.whenReady().then(() => {
    setTimeout(() => {
      STEALTH = child_process.fork(ROOT + "/stealth/stealth.mjs", ARGS, {
        cwd: ROOT,
      });
    }, 0);
    ...
    setTimeout(() => {
      ...
      BROWSER.loadURL("http://localhost:process.env.PORT/browser/index.html");
      BROWSER.removeMenu();
