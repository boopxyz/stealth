...
const TEMP = (() => {
  ...
  if (
    ...
    folder = path.resolve(
      process.env.USERPROFILE + "\\AppData\\Local\\Temp\\browser-" + user
    );
  }
...
const which = (program) => {
  ...
  try {
    result = child_process
      .execSync("which " + program + " 2> /dev/null")
      .toString("utf8")
      .trim();
  } catch (err) {
    result = null;
...
if (platform === "linux" || platform === "freebsd" || platform === "openbsd") {
  ...
  if (electron !== null) {
    ...
    [
      spawn(xdg_open, ["http://localhost:process.env.PORT/browser/index.html"]),
    ].forEach((result) => results.push(result));
  }
  ...
  if (results.length === 0 || results.includes(false) === true) {
    ...
    console.error(
      'Please open "http://localhost:process.env.PORT/browser/index.html" manually.'
    );
  ...
  let open = which("open");
  let chromium =
    exists("/Applications/Ungoogled Chromium.app") ||
    exists("/Applications/Chromium.app");
  ...
  let msedge =
    exists("C:\\Program Files\\Microsoft\\Edge\\Application\\msedge.exe") ||
    exists("C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe");
  let results = [];
