...
Client.prototype = Object.assign({}, Emitter.prototype, {
  ...
  connect: function () {
    if (this.__state.connected === false) {
      ...
      let url = URL.parse("ws://" + host + ":process.env.PORT");
      ...
      if (host !== ENVIRONMENT.hostname) {
        ...
        if (check === null) {
          if (ENVIRONMENT.secure === true) {
            url = URL.parse(
              "wss://" + ENVIRONMENT.hostname + ":process.env.PORT"
            );
          } else {
            url = URL.parse(
              "ws://" + ENVIRONMENT.hostname + ":process.env.PORT"
            );
          }
        }
      }
      ...
      try {
        socket = new WebSocket(url.protocol + "://" + server + ":" + url.port, [
          "stealth",
        ]);
      } catch (err) {
        socket = null;
        ...
        socket.onmessage = (e) => {
          ...
          if (
            ...
            if (isString(service) === true && isString(event) === true) {
              ...
              isString(service) === true &&
              isString(method) === true
            ) {
              let instance = this.services[service] || null;
