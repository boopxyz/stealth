  ...
  connect: function () {
    if (this.__state.connected === false) {
      ...
      this.__state.server.on("connection", (socket) => {
        socket.once("data", (buffer) => {
          if (buffer[0] === 0x16) {
            ...
            if (version !== null) {
              socket.write(
                "🛡️ TLS Version " + version + " not yet supported. Stay tuned!"
              );
            }
            ...
            this.services !== null &&
            this.services.can(buffer) === true
          ) {
            this.services.upgrade(buffer, socket);
          } else if (
            this.webproxy !== null &&
            this.webproxy.can(buffer) === true
          ) {
            this.webproxy.upgrade(buffer, socket);
          } else if (
            this.webserver !== null &&
            this.webserver.can(buffer) === true
          ) {
            this.webserver.upgrade(buffer, socket);
          } else if (
            this.defender !== null &&
            this.defender.can(buffer) === true
          ) {
      ...
      this.__state.server.on("listening", () => {
        let host = isString(this._settings.host)
          ? this._settings.host
          : "localhost";

        console.info(
          "Server: TCP Service for http+socks+ws://" +
            host +
            ":process.env.PORT started."
        );
        console.info("Server: > http://" + host + ":process.env.PORT");
        ...
        if (ENVIRONMENT.ips.length > 0) {
          ENVIRONMENT.ips.forEach((ip) => {
            ...
            if (hostname !== "localhost") {
              console.info(
                "Server: > http://" + hostname + ":process.env.PORT"
              );
            }
          });
        }
      ...
      this.__state.server.listen(process.env.PORT, host);
