...
const isStatus = function (status) {
  if (isString(status) === true && isNumber(STATUSES[status]) === true) {
    return true;
  }
    ...
    if (
      ...
      if (connection.has("@connect-tunnel") === true) {
        ...
        SOCKS.send(connection, {
          ...
          payload: URL.parse("127.0.0.1:process.env.PORT"),
        });
  ...
  if (isObject(json) === true) {
    ...
    if (json instanceof Connection === true) {
      ...
      Object.keys(connection).forEach((property) => {
        if (json[property] === undefined || json[property] === null) {
...
Connection.prototype = Object.assign({}, Emitter.prototype, {
  ...
  toJSON: function () {
    ...
    if (this.socket !== null) {
      ...
      if (IP.isIP(local.host) === true && isNumber(local.port) === true) {
        data.local = local;
      }
      ...
      if (IP.isIP(remote.host) === true && isNumber(remote.port) === true) {
        data.remote = remote;
      }
    }
...
const SOCKS = {
  connect: function (url, connection) {
    ...
    connection = isConnection(connection)
