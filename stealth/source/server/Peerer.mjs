...
const toOptionValue = (packet, key) => {
  let txt =
    packet.payload.answers.find((record) => {
      if (
        record.type === "TXT" &&
...
const isQuery = function (payload) {
  if (
    ...
    if (
      ...
      let domain1 = isString(payload.subdomain)
...
const isServiceDiscoveryRequest = function (packet) {
  if (
    ...
    let ptr1 =
      packet.payload.questions.find(
        (q) => q.type === "PTR" && q.value === "_stealth._wss.tholian.local"
      ) || null;
    let ptr2 =
      packet.payload.questions.find(
        (q) => q.type === "PTR" && q.value === "_stealth._ws.tholian.local"
      ) || null;
    if (ptr1 !== null || ptr2 !== null) {
      let rest = packet.payload.questions.filter(
        (q) => q !== ptr1 && q !== ptr2
      );
      if (rest.length === 0) {
        return true;
...
const toServiceDiscoveryResponse = function (request) {
  ...
  if (this.stealth !== null) {
    if (
      ...
      isBuffer(this.stealth.settings.account["certificate"]["private"]) ===
        true &&
      isBuffer(this.stealth.settings.account["certificate"]["public"]) ===
        true &&
      isString(this.stealth.settings.account["username"]) === true
    ) {
      certificate =
        this.stealth.settings.account["certificate"]["public"].toString(
          "base64"
        );
  ...
  if (isString(hostname) === true && ENVIRONMENT.ips.length > 0) {
    ...
    response.payload.answers.push({
      type: "TXT",
      domain: hostname,
      value: [Buffer.from("version=" + VERSION, "utf8")],
    });

    response.payload.answers.push({
      domain: hostname,
      type: "TXT",
      value: [Buffer.from("certificate=" + certificate, "utf8")],
    });

    response.payload.answers.push({
      domain: hostname,
      type: "TXT",
      value: [Buffer.from("connection=" + connection, "utf8")],
    });
...
Peerer.prototype = {
  ...
  connect: function () {
    ...
    if (has_ipv4 === true) {
      ...
      if (connection === null) {
        ...
        connection.once("@connect", () => {
          console.info(
            "Peerer: UDP Service for mdns://224.0.0.251:5353 started."
          );
        ...
        connection.once("disconnect", () => {
          console.warn(
            "Peerer: UDP Service for mdns://224.0.0.251:5353 stopped."
          );
        });
    ...
    if (has_ipv6 === true) {
      ...
      if (connection === null) {
        ...
        connection.once("@connect", () => {
          console.info(
            "Peerer: UDP Service for mdns://[ff02::fb]:5353 started."
          );
        ...
        connection.once("disconnect", () => {
          console.warn(
            "Peerer: UDP Service for mdns://[ff02::fb]:5353 stopped."
          );
        });
    ...
    if (isServiceDiscoveryRequest(packet) === true) {
      if (this.stealth !== null && this.stealth._settings.debug === true) {
        ...
        if (remote !== null) {
          console.log(
            'Peerer: Client "' +
              IP.render(remote["host"]) +
              '" sent a Multicast DNS-SD Request.'
          );
        }
      }
      ...
      if (this.stealth !== null && this.stealth._settings.debug === true) {
        ...
        if (remote !== null) {
          console.log(
            'Peerer: Client "' +
              IP.render(remote["host"]) +
              '" sent a Multicast DNS-SD Response.'
          );
        }
      }
      ...
      if (this.stealth !== null) {
        if (this.services !== null) {
          ...
          let srv =
            packet.payload.answers.find((a) => a.type === "SRV") || null;
          if (srv !== null) {
          ...
          let ipv4s = packet.payload.answers
            .filter((a) => a.type === "A")
            .map((a) => a.value);
          if (ipv4s.length > 0) {
          ...
          let ipv6s = packet.payload.answers
          ...
          if (host.domain !== null && peer.domain !== null) {
            if (
              peer.domain.endsWith(".tholian.local") &&
              peer.peer.certificate !== null
            ) {
              console.info(
                'Peerer: Trusted Peer "' + peer.domain + '" updated.'
              );
            } else {
              console.warn(
                'Peerer: Untrusted Peer "' + peer.domain + '" updated.'
              );
            }
                  ...
                  this.services.peer.read(
                    ...
                    (response) => {
                      if (response.payload !== null) {
                        if (
                          response.payload.peer.certificate ===
  ...
  resolve: function (query, callback) {
    ...
    if (query !== null && ENVIRONMENT.ips.length > 0) {
      ...
      if (username !== null) {
        ...
        if (domain === "tholian.local") {
          if (this.stealth !== null) {
            let host =
              this.stealth.settings.hosts.find(
                (h) => h.domain === username + ".tholian.local"
              ) || null;
            if (host !== null) {
              if (callback !== null) {
                callback(host);
