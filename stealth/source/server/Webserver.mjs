  ...
  if (
    ...
    if (isString(request.headers["accept-encoding"]) === true) {
      if (request.headers["accept-encoding"].includes(",") === true) {
        let candidates = request.headers["accept-encoding"]
          .split(",")
          .map((v) => v.trim());
    ...
    if (isArray(request.headers["@transfer"]["range"]) === true) {
      if (
        ...
        response.headers["@transfer"]["range"] =
          request.headers["@transfer"]["range"];
  ...
  if (url.path === "/") {
    ...
    toFile(url, (response) => {
      response.headers["Content-Security-Policy"] =
        "worker-src 'self'; script-src 'self' 'unsafe-inline'; frame-src 'self'";
    ...
    if (domain !== null) {
      pac_url = URL.parse("http://" + domain + ":process.env.PORT/proxy.pac");
    } else if (host !== null) {
      pac_url = URL.parse("http://" + host + ":process.env.PORT/proxy.pac");
    } else {
      pac_url = URL.parse(
        "http://" + ENVIRONMENT.hostname + ":process.env.PORT/proxy.pac"
      );
    }
              ...
              if (this.stealth._settings.debug === true) {
                ...
                if (host !== null) {
                  if (this.__state.connections[host] === undefined) {
                    ...
                    isNumber(this.__state.connections[host]) === true
                  ) {
          ...
          connection.on("request", (request) => {
            ...
            if (isString(request.headers["@remote"]) === true) {
              if (request.headers["@remote"].startsWith("::ffff:") === true) {
                request.headers["@remote"] =
                  request.headers["@remote"].substr(7);
              }
            }
                ...
                if (host !== null) {
                  ...
                  setTimeout(() => {
                    if (this.__state.connections[host] === 0) {
                      console.log(
                        'Webserver: Client "' + host + '" disconnected.'
                      );
