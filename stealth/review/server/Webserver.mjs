...
describe("Webserver.prototype.upgrade()/GET/", function (assert) {
  ...
  let url = URL.parse("http://localhost:process.env.PORT");
...
describe("Webserver.prototype.upgrade()/GET/proxy.pac/127.0.0.1", function (assert) {
  ...
  let url = URL.parse("http://127.0.0.1:process.env.PORT");
  let connection = HTTP.connect(url);
  ...
  connection.once("response", (response) => {
    ...
    assert(
      response.payload
        .toString("utf8")
        .includes('(host === "' + ENVIRONMENT.hostname + '")')
    );
    assert(
      response.payload
        .toString("utf8")
        .includes('(host === "' + url.host + '")')
    );
...
describe("Webserver.prototype.upgrade()/GET/proxy.pac/127.0.0.2", function (assert) {
  ...
  let url = URL.parse("http://127.0.0.2:process.env.PORT");
  let connection = HTTP.connect(url);
